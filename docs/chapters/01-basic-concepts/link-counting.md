...
??? note "Примечание"
    ...
    Обратите внимание, что код из [листинга 1-9](<#l-1-9>) не соответствует лучшим практикам C++. Передача интеллектуального указателя только для чтения базовых данных должна осуществляться с помощью константной ссылки (const&), а не значения. Мы решили не делать этого в нашем примере, чтобы продемонстрировать, как копирование увеличивает счетчик ссылок. Мы видим значительное улучшение в этой версии кода, потому что
