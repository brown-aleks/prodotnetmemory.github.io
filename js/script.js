document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadNavigation() {
        fetch('navigation.html')
            ...
            .then(html => {
                ...
                document.querySelector('#navbarScroll').innerHTML = doc.getElementById('sidebarNavContent').innerHTML;
    ...
    function initOffcanvas() {
        setTimeout(() => {
            var offcanvasElement = document.getElementById('offcanvasNavbarContainer');
            if (offcanvasElement) {
                offcanvasInstance = new bootstrap.Offcanvas(offcanvasElement);

                // Добавляем обработчик для закрытия Offcanvas при клике на ссылку внутри него
                document.querySelectorAll('#offcanvasNavbarContainer .nav-link').forEach(link => {
